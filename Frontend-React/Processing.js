...
function processOrderList(orderList, orderId, state) {
  ...
  // If the order with the given orderId is not found, return the original orderList
  if (orderIndex === -1) {
    return orderList;
  }

  // Depending on the state, update or delete the order
  if (state === 'Processing') {
    // Update the state of the order to "Processing"
